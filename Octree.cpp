...
//
void Octree::draw(TreeNode & node, int numLevels, int lvl) {
	if (lvl >= numLevels) return;
    ofSetColor(lvl*25, 90, (lvl%2)*250);
	drawBox(node.box);
	lvl++;
	for (int i = 0; i < node.children.size(); i++) {
		draw(node.children[i], numLevels, lvl);
	}
...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	// call subdivide method
    cout << "Start Octree" <<endl;
	root.box = meshBounds(geo);
	for(int i = 0; i < geo.getNumVertices(); i++){
		root.points.push_back(i);
	}
	subdivide(geo, root, numLevels, 0);
    cout << "Finish Octree" <<endl;
}


//TODO
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int lvl) {
	if(lvl < numLevels && node.points.size() > 0){
		vector<Box> subBox;
		subDivideBox8(node.box, subBox);
		for(int i = 0; i < 8; i++){
			vector<int> pointsRtn;
			if(getMeshPointsInBox(mesh, node.points, subBox[i], pointsRtn) > 0){
				TreeNode newNode;
				newNode.box = subBox[i];
				newNode.points = pointsRtn;
				if(pointsRtn.size() > 1)
					subdivide(mesh, newNode, numLevels, lvl+1);
...
bool Octree::intersect(const Ray &r, const TreeNode & node, TreeNode & nodeRtn) {
	if(node.box.intersect(r, -10000, 10000)){
		...
		for(int i = 0; i < node.children.size(); i++){
            bool ret = intersect(r, node.children[i], nodeRtn);
			if(ret)
                return true;
		}
	}
